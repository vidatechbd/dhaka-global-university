...
<x-dynamic-component :component="$layout">
    ...
    <div class="flex flex-col gap-6 w-full">
        ...
        <!-- Dynamic Academic Transcript & Certificate Edit Form -->
        <form id="marksheet-form" method="POST" action="{{ route('marksheets.update', $marksheet) }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <section id="form-section" class="bg-white border border-slate-200 p-8 rounded-3xl shadow-sm no-print">
                <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6 pb-4 border-b border-slate-100">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-xl font-bold text-primary border-l-4 border-secondary pl-3">Edit Academic Transcript</h2>
                    </div>
                    <div class="flex items-center gap-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    <div>
                        ...
                        <select id="student_id" name="student_id" onchange="autoFillStudent(this)" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" data-name="{{ $student->name }}" data-email="{{ $student->email }}" class="bg-white" {{ (old('student_id', $marksheet->student_id) == $student->id) ? 'selected' : '' }}>{{ $student->name }} ({{ $student->email }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $marksheet->title) }}" oninput="updateTranscriptPreview()" id="input-title" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-bold" required>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Student's Name</label>
                        <input type="text" name="student_name" id="input-student-name" value="{{ old('student_name', $marksheet->student_name) }}" oninput="updateTranscriptPreview()" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-bold" required>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Father's Name</label>
                        <input type="text" name="father_name" value="{{ old('father_name', $marksheet->father_name) }}" oninput="updateTranscriptPreview()" id="input-father" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Mother's Name</label>
                        <input type="text" name="mother_name" value="{{ old('mother_name', $marksheet->mother_name) }}" oninput="updateTranscriptPreview()" id="input-mother" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Name of Course</label>
                        <input type="text" name="course_name" value="{{ old('course_name', $marksheet->course_name) }}" oninput="updateTranscriptPreview()" id="input-course" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Exam. Roll</label>
                        <input type="number" name="exam_roll" value="{{ old('exam_roll', $marksheet->exam_roll) }}" oninput="updateTranscriptPreview()" id="input-roll" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Registration No</label>
                        <input type="text" name="reg_no" value="{{ old('reg_no', $marksheet->reg_no) }}" oninput="updateTranscriptPreview()" id="input-reg" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Session</label>
                        <input type="text" name="session" value="{{ old('session', $marksheet->session) }}" oninput="updateTranscriptPreview()" id="input-session" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Subject/Department Name</label>
                        <input type="text" name="department" value="{{ old('department', $marksheet->department) }}" oninput="updateTranscriptPreview()" id="input-dept" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Credits Completed</label>
                        <input type="text" name="credit_completed" value="{{ old('credit_completed', $marksheet->credit_completed) }}" oninput="updateTranscriptPreview()" id="input-credit" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="result" placeholder="3.96 Out Of 4.00" value="{{ old('result', $marksheet->result) }}" oninput="updateTranscriptPreview()" id="input-result" class="w-full bg-white border border-slate-300 text-[#d97d10] rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-bold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Date of Issue</label>
                        <input type="date" name="date_of_issue" value="{{ old('date_of_issue', $marksheet->date_of_issue ? (\Carbon\Carbon::parse($marksheet->date_of_issue)->format('Y-m-d')) : '') }}" oninput="updateTranscriptPreview()" id="input-date-of-issue" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Result Published</label>
                        <input type="date" name="result_published" value="{{ old('result_published', $marksheet->result_published ? (\Carbon\Carbon::parse($marksheet->result_published)->format('Y-m-d')) : '') }}" oninput="updateTranscriptPreview()" id="input-result-published" class="w-full bg-white border border-slate-300 text-slate-800 rounded-lg px-4 py-2.5 outline-none focus:border-primary focus:ring-1 focus:ring-primary text-xs font-semibold">
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <div class="flex-1 text-center">
                                ...
                                <h2 class="text-[14px] font-bold underline underline-offset-2 mt-1" id="preview-title-heading">ACADEMIC TRANSCRIPT</h2>
                            </div>
                            <div class="w-20"></div>
                        </div>
                        ...
                        <div class="mb-3 text-[12px] leading-[1.3] w-[85%]">
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Student's Name</div><div class="w-4 font-bold">:</div><div id="preview-name" class="font-bold uppercase"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Father's Name</div><div class="w-4 font-bold">:</div><div id="preview-father" class="font-bold uppercase"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Mother's Name</div><div class="w-4 font-bold">:</div><div id="preview-mother" class="font-bold uppercase"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Name of Course</div><div class="w-4 font-bold">:</div><div id="preview-course" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Exam. Roll</div><div class="w-4 font-bold">:</div><div id="preview-roll" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Registration No</div><div class="w-4 font-bold">:</div><div id="preview-reg" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Session</div><div class="w-4 font-bold">:</div><div id="preview-session" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Subject/Department Name</div><div class="w-4 font-bold">:</div><div id="preview-dept" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Credit (Completed/Total)</div><div class="w-4 font-bold">:</div><div id="preview-credit" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Result</div><div class="w-4 font-bold">:</div><div id="preview-result" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Date of Issue</div><div class="w-4 font-bold">:</div><div id="preview-date-of-issue" class="font-bold"></div></div>
                            <div class="flex mb-[1px]"><div class="w-48 font-bold">Result Published</div><div class="w-4 font-bold">:</div><div id="preview-result-published" class="font-bold"></div></div>
                        </div>
        ...
        <script>
            let stateSemesters = @json($marksheet->semesters ?? []);
            ...
            function updateTranscriptPreview() {
                document.getElementById('preview-title-heading').textContent = (document.getElementById('input-title').value || 'ACADEMIC TRANSCRIPT').toUpperCase();
            ...
            function saveSemester() {
                const yearInput = document.getElementById('sem-name-input').value.trim().toUpperCase();
                const gpaInput = document.getElementById('sem-gpa-input').value.trim();
                
                if (!yearInput || !gpaInput) {
                    alert('Please enter Year name and Year CGP');
                    return;
                }
                ...
                courseRows.forEach(row => {
                    ...
                    const credit = row.querySelector('.course-credit').value.trim();
                    const grade = row.querySelector('.course-grade').value;

                    if (!code || !title || !credit) {
                        hasEmpty = true;
                    }
                    courses.push({ code, title, credit, grade });
                });

                if (hasEmpty) {
                    alert('Please fill out all course details or delete empty rows.');
                    return;
                }
                ...
                if (editingIndex > -1) {
                    // Update
                    stateSemesters[editingIndex] = newSem;
                } else {
                    // Create
                    stateSemesters.push(newSem);
                }
