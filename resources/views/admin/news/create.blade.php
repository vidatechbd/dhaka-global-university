<x-admin-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3 w-full">
            <div>
                <h1 class="text-xl font-bold text-primary">{{ __('Create News Article') }}</h1>
                <p class="text-xs text-slate-500 mt-0.5">Write and publish a new campus news article.</p>
            </div>
    ...
    <div class="max-w-4xl">
        <x-admin.card title="Article Details" subtitle="Fill in the details below to create a new article." icon="ph-bold ph-newspaper-clipping">
            <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input id="title" class="mt-2 w-full" type="text" name="title" :value="old('title')" placeholder="e.g. Fall Semester Registration Opens" required autofocus />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <label for="thumbnail" class="mt-2 flex items-center justify-center gap-2 px-4 py-6 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 hover:border-primary/50 hover:bg-[#f6fafc] cursor-pointer transition-colors text-slate-500">
                            ...
                            <span class="text-xs font-semibold">Click to upload an image</span>
                        </label>
