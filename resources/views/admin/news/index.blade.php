<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-bold text-gray-800">{{ __('News Articles') }}</h1>
            <a href="{{ route('admin.news.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg shadow transition-colors">
                + Create News
            </a>
        </div>
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-sm font-bold text-gray-700">{{ __('All Articles') }}</h2>
        </div>
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4 text-center">
                                @if($article->thumbnail)
                                    <img src="{{ asset($article->thumbnail) }}" alt="Thumbnail" class="w-12 h-8 object-cover rounded-md border border-gray-100 mx-auto">
                                @else
                            ...
                            <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                <a href="{{ route('admin.news.edit', $article) }}" class="inline-block px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 font-bold rounded transition">
                                    Edit
                                </a>
