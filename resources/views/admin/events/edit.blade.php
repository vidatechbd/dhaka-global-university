<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <h1 class="text-xl font-bold text-gray-800">{{ __('Edit Event') }}</h1>
        </div>
    </x-slot>
        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    @if($event->thumbnail)
                        <div class="mt-2 mb-2 flex items-center gap-3">
                            <img src="{{ asset($event->thumbnail) }}" alt="Thumbnail" class="w-20 h-14 object-cover rounded-lg border border-gray-200">
                ...
                <div>
                    ...
                    <input id="event_date" type="date" name="event_date" value="{{ old('event_date', $event->event_date ? $event->event_date->format('Y-m-d') : '') }}" class="mt-1 block w-full border border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm px-3 py-2 text-xs bg-white">
