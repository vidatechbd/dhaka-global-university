<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-bold text-gray-800">{{ __('Create Sidebar') }}</h1>
    ...
    <div x-data="{
        ...
        <form action="{{ route('admin.sidebars.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-300 p-6 space-y-6">
            @csrf

            <!-- Sidebar Name -->
            <div>
                <x-input-label for="name" :value="__('Sidebar Name')" />
                <x-text-input id="name" name="name" class="block mt-1 w-full border border-gray-300 rounded-md px-3 py-2" type="text" required :value="old('name')" placeholder="e.g. Admission Side Nav" />
