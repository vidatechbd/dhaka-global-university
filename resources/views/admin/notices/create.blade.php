<x-admin-layout>
    <x-slot name="header">
        <h1 class="text-xl font-bold text-gray-800">{{ __('Create Notice') }}</h1>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.notices.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <x-text-input id="title" class="block mt-1 w-full border border-gray-300 rounded-md px-3 py-2" type="text" name="title" :value="old('title')" required autofocus />
                <x-input-error :messages="$errors->get('title')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="content" :value="__('Notice Content')" />
                <textarea id="content" name="content" rows="6" class="mt-1 block w-full border border-gray-300 rounded-md p-3 text-xs outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>{{ old('content') }}</textarea>
