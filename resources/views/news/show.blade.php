<x-home-layout>
    ...
    <div class="bg-slate-100 border-b border-slate-200 py-4 no-print">
        <div class="container mx-auto px-4 md:px-6 flex items-center justify-between text-xs text-slate-500 font-semibold">
            ...
            <a href="{{ route('news.index') }}" class="hover:text-primary transition flex items-center gap-1 font-bold text-primary">
                &larr; Back to News
            </a>
        </div>
    </div>
                ...
                <article class="lg:col-span-8 space-y-6">
                    <header class="space-y-4">
                        ...
                        <div class="flex items-center gap-4 text-xs text-slate-400 border-b border-slate-100 pb-4 font-semibold">
                            ...
                            <span class="flex items-center gap-1">
                                <i class="ph ph-calendar"></i> {{ $news->created_at->format('h:i A') }}
                            </span>
                        </div>
                    </header>
                    ...
                    @if($news->thumbnail)
                        <div class="w-full max-h-[450px] overflow-hidden bg-slate-100 border border-slate-200">
                            <img src="{{ asset($news->thumbnail) }}" alt="{{ $news->title }}" class="w-full h-auto object-cover">
                        </div>
                    @endif
                    ...
                    <div class="bg-slate-50 p-6 border border-slate-200 shadow-sm">
                        ...
                        <div class="space-y-4">
                            @forelse($recentNews as $recent)
                                <a href="{{ route('news.show', $recent) }}" class="flex gap-3 group items-center">
                            ...
                            @empty
                                <p class="text-slate-400 text-xs text-center py-4">No recent articles found.</p>
                            @endforelse
                        </div>
                    </div>
                </aside>
