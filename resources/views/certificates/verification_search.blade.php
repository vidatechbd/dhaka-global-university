...
<body class="bg-gradient-to-tr from-[#f1f5f9] to-[#e2e8f0] font-sans min-h-screen flex flex-col items-center justify-center p-4">
    ...
    <div class="w-full max-w-md bg-white rounded-3xl shadow-xl border border-slate-200/80 overflow-hidden transition-all duration-300 hover:shadow-2xl">
        ...
        <!-- Form Section -->
        <form action="{{ route('verification.search') }}" method="POST" class="p-8 space-y-6">
            @csrf

            <div class="text-center">
                <h3 class="text-base font-bold text-slate-800">Certificate Verification</h3>
                <p class="text-xs text-slate-400 mt-1">Please enter the Registration Number and Exam Roll Number to search for and verify the certificate.</p>
            </div>
